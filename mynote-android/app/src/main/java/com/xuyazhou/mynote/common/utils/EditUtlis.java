...
public class EditUtlis {
    ...
    private TextUndoListener listener;

    public void setListener(TextUndoListener listener) {
        this.listener = listener;
    }

    public EditUtlis(@NonNull EditText editText) {
    ...
    private class Watcher implements TextWatcher {
        ...
        public final void afterTextChanged(Editable s) {
            ...
            EditUtlis.this.onTextChanged(s);
    ...
    private void changeUnDoEnable() {
        if (history.size() > 0) {
            EditUtlis.this.listener.undo(true);
        } else {
            EditUtlis.this.listener.undo(false);
        }

        if (historyBack.size() > 0) {
            EditUtlis.this.listener.redo(true);
        } else {
            EditUtlis.this.listener.redo(false);
        }
    }
    ...
    public interface TextUndoListener {
        void undo(boolean enable);

        void redo(boolean enable);
    }
}
